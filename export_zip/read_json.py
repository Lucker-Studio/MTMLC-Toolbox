...
def read_json(json_path: str) -> list:
    ...
    for note_id, note in enumerate(project_data['note_list']):
        ...
        add_cmd = [0]*10  # 初始化长度为 10 的数组
        add_cmd[0] = note_id  # note 的 ID
        add_cmd[1] = sum(1 << i for i, j in enumerate(NOTE_PROPERTIES) if note['properties'].get(j, False))  # 用 2 的整数次幂表示 note 的属性
        add_cmd[2:5] = map(float, key_points_abc.pop(0)[1:])  # 初始位置函数
        add_cmd[5] = note['initial_showing_track']  # 初始显示轨道
        add_cmd[6] = note['judging_track']  # 实际判定轨道
        add_cmd[7] = float(start_time)  # 开始时间
        add_cmd[8] = float(end_time)  # 结束时间
        add_cmd[9] = float(end_pos-start_pos)  # 显示长度
        commands.append((-PREACTIVATING_TIME, ADD_NOTE, add_cmd))  # 添加 note 指令
        ...
        for t in showing_track_changes_processed:
            commands.append((t, CHANGE_NOTE_TRACK, showing_track_changes_processed[t]))  # 改变 note 轨道函数指令

    commands.append((-PREACTIVATING_TIME, CHANGE_LINE_POS, (LINEAR_SLOW_MOVING, 0, project_data['line']['initial_position'])))
    ...
    for t in line_motions_processed:
        commands.append((t, CHANGE_LINE_POS, line_motions_processed[t]))  # 改变判定线位置函数指令

    return sorted(commands)  # 将指令按时间排序
