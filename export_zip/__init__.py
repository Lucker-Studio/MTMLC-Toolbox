...
def main() -> None:
    ...
    using_template = easygui.ynbox('是否要使用已有的模板？', '导出 OMGZ 文件', ('是', '否'))
    ...
    if not skip:
        ...
        while True:
            data = easygui.multenterbox('请输入歌曲信息：', '导出 OMGZ 文件', ['曲名', '曲师', '画师'], data)
        ...
        if len(charts_info) >= 1:  # 至少添加一张谱面
            ...
            if ch is None:  # 关闭对话框
                ...
                if omgz_path is None:  # 未保存文件
                    return
                write_zip(music_path, illustration_path, title, composer, illustrator, charts_info, omgz_path)
