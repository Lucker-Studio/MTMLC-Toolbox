...
from .make_omgz import make_omgz
...
def main() -> None:
    ...
    if easygui.ynbox('是否要使用已有的模板？', '导出项目', ('是', '否')):
        pickle_path = easygui.fileopenbox('请选择生成设置文件', '导出项目', '*.pkl', ['*.pkl'])
        try:
    ...
    music_path = easygui.fileopenbox(
        '请选择歌曲音频', '导出项目', music_path, ['*.ogg'])
    if not music_path:
        return
    illustration_path = easygui.fileopenbox(
        '请选择曲绘图片', '导出项目', illustration_path, ['*.png'])
    ...
    while True:
        if charts_info:  # 至少添加一张谱面
            ...
            ch = easygui.buttonbox(f'已添加 {len(charts_info)} 张谱面：\n'+'\n'.join(show_charts.keys()), '导出项目', ('继续添加', '完成添加', '删除谱面'))
            if not ch:
                return
            if ch == '完成添加':
                omgz_path = easygui.filesavebox('保存 OMGZ 文件', '导出项目', title+'.omgz',  ['*.omgz'])
                if omgz_path:
                    try:
                        make_omgz(music_path, illustration_path, title, composer, illustrator, charts_info, omgz_path)
                        easygui.msgbox('导出成功！', '导出项目', '好耶')
                        if easygui.ynbox('是否保存模板供以后使用？', '导出项目', ('是', '否')):
                            pickle_path = easygui.filesavebox('保存生成设置', '导出项目', title+'.pkl', ['*.pkl'])
                            if pickle_path:
                        ...
                        easygui.exceptionbox('出错啦！', '导出项目')
                    return
            if ch == '删除谱面':
                charts_to_delete = easygui.multchoicebox('请选择要删除的谱面（支持多选）', '导出项目', show_charts.keys())
        ...
        data = ['']*3
        while True:
            data = easygui.multenterbox(f'请输入第 {len(charts_info)+1} 张谱面信息：', '导出项目', ['难度', '定数', '谱师'], data)
            if not data:
                if charts_info:
                    break
                else:
