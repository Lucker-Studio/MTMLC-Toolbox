    ...
    while True:
        ...
        if omgc_data is None:
            if len(chart_choices) > 1:
                ...
                return
        while True:
            input_data = easygui.multenterbox('请确认或更改播放参数', f'{title} - 谱面预览', ['流速倍率(不小于1)', '音乐音量(0~1之间)'], [PREVIEW_DEFAULT_NOTE_SPEED_RATE, PREVIEW_DEFAULT_MUSIC_VOLUME])
        ...
        game = Game(omgc_data, os.path.join(chart_dir, 'music.mp3'), game_window, note_speed_rate, music_volume)
        game.main_loop()
        if len(chart_choices) == 1:
            return
